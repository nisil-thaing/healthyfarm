...
class TRX_Importer {
	...
	var $options = array(
		...
		'file_with_royal_sliders' => '',	// Name of file with sliders data
		...
		'page_on_front'		=> 'Home5',						// Homepage title
		'page_for_posts'	=> '',							// Blog streampage title
		'menus'				=> array(						// Menus locations and names
			'mainmenu'	=> 'mainMenu',
		),
		'wooc_pages'		=> 	array(						// Options slugs and pages titles for WooCommerce pages
			'woocommerce_shop_page_id' => 'Shop', // 'Featured Products'
			...
			<?php if (empty($this->success) && $this->options['enable_importer']) { ?>
				...
				<form id="trx_importer_form" action="#" method="post">
					...
					<div class="trx_buttons">
						<input type="submit" class="button button-primary customize load-customize hide-if-no-customize" value="<?php _e('Start Import', 'themerex'); ?>">
						<?php if ($this->import_last_id > 0) { ?>
						<input type="submit" class="button action" value="<?php printf(__('Continue Import (from ID=%s)', 'themerex'), $this->import_last_id); ?>" onClick="this.form.last_id.value='<?php echo $this->import_last_id; ?>'">
						<?php } ?>
					</div>
			...
			<?php if (empty($this->success) && $this->options['enable_exporter']) { ?>
				...
				<form id="trx_exporter_form" action="#" method="post">
					...
					<div class="trx_buttons">
						...
						<?php } else { ?>
						<input type="submit" class="button action" value="<?php _e('Export Theme Options', 'themerex'); ?>">
						<?php } ?>
					</div>
...
if ($this->options['debug']) { echo ' '.__('import error - ', 'themerex').$response['error'].'<br>';} 
						} else {
