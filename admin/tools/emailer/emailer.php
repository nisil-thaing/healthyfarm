...
class TRX_Emailer {
	...
	function admin_menu_item() {
		$menu_page = add_theme_page(__('Emailer', 'themerex'), __('Emailer', 'themerex'), 'manage_options', 'trx_emailer', array($this, 'build_page'));
	}
	...
	function load_styles() {
		if (isset($_REQUEST['page']) && $_REQUEST['page']=='trx_emailer') {
			themerex_enqueue_style('trx-emailer-style', get_template_directory_uri() . '/admin/tools/emailer/emailer.css', array(), null);
		}
	}
	...
	function build_page() {
		...
		$subscribers_update = $subscribers_delete = false;
		$subscribers = array();
		if ( isset($_POST['emailer_subject']) ) {
			do {
				...
				$sender_name = isset($_POST['emailer_sender_name']) ? $_POST['emailer_sender_name'] : '';
				$sender_email = isset($_POST['emailer_sender_email']) ? $_POST['emailer_sender_email'] : '';
				if (empty($sender_email)) $sender_email = get_theme_option('contact_email');
				if (empty($sender_email)) $sender_email = get_option('admin_email');
				...
				if ($subscribers_update && $group!='none') {
					$this->subscribers[$group] = $new_list;
								...
								<select name="emailer_group" id="emailer_group">
									...
									if (count($this->subscribers) > 0) {
										foreach ($this->subscribers as $gr=>$list) {
											echo balanceTags('<option value="'.$gr.'"'.($group==$gr ? ' selected="selected"' : '').'>'.themerex_strtoproper($gr).'</option>');
										}
									}
									?>
								</select>
