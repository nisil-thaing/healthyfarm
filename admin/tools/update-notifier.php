...
define( 'NOTIFIER_THEME_NAME', 'Healthy Farm' ); // The theme name
define( 'NOTIFIER_THEME_FOLDER_NAME', 'Healthy Farm' ); // The theme folder name
define( 'NOTIFIER_XML_FILE', 'http://themerex.net/!updates/updates_healthyfarm.xml' ); // The remote notifier XML file containing the latest version of the theme and changelog
...
function update_notifier_menu() {  
	if (function_exists('simplexml_load_string')) { // Stop if simplexml_load_string function isn't available
		...
		if( (string) $xml->latest > (string) $theme_data->get('Version') ) { // Compare current theme version with the remote XML version
			//add_theme_page( NOTIFIER_THEME_NAME . ' ' . __('Theme Updates', 'themerex'), NOTIFIER_THEME_NAME . ' <span class="update-plugins count-1"><span class="update-count">'.__('New Update', 'themerex').'</span></span>', 'administrator', 'theme-update-notifier', 'update_notifier');
		}
	}	
}
...
?>
	...
	<div class="wrap">
		...
		<h2><?php echo NOTIFIER_THEME_NAME ?> <?php echo _e('Theme Updates', 'themerex'); ?></h2>
		...
		<div><img style="margin: 0 0 20px 0; border: 1px solid #ddd;" src="<?php echo esc_url(get_template_directory_uri()).'/screenshot.png'; ?>" /></div>
