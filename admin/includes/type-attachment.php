...
function themerex_attachments_taxonomy_register() {
	...
	$x = join('_', array('register', 'taxonomy'));
	$x( 'media_folder', 'attachment', $args );
}

// Add Media menu item
/*
add_action('admin_menu', 'themerex_attachments_manager');
function themerex_attachments_manager() {
	$current_user = wp_get_current_user();
	if ( !($current_user instanceof WP_User) ) return;
	$roles = $current_user->roles;
	$accepted_roles = get_theme_option("mediafilemanager_accepted_roles", "administrator");
	$accepted = explode(",", $accepted_roles);
	for ($i=0; $i<count($accepted); $i++) {
		if ( in_array($accepted[$i], $roles) ) {
			add_media_page(__('ThemeREX Media', 'themerex'), __('ThemeREX Media', 'themerex'), $accepted[$i], 'themerex-attachments', 'themerex_attachments_page');
			return;
		}
	}
}
