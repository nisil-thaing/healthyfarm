...
jQuery(document).ready(function(){
	...
	jQuery(".sc_selector").change(function() {
		...
		var hdr = sc.title;
		var content = tinyMCE.activeEditor ? tinyMCE.activeEditor.selection.getContent({format : 'raw'}) : jQuery('#wp-content-editor-container textarea').selection();
		if (content) {
		...
		THEMEREX_shortcodes_popup = themerex_message_dialog(html, hdr,
			...
			function(btn, popup) {
				"use strict";
				if (btn != 1) return;
				send_to_editor(themerex_shortcodes_get_code(THEMEREX_shortcodes_popup));
			});
...
function themerex_shortcodes_get_code_from_tab(tab) {
	...
	tab.find('[data-param]').each(function () {
		...
		if (param.substr(0, 1)!='_' && !empty(field.val()) && field.val()!='none') {
	...
	if (tab.attr('id').indexOf('tab_0') > 0) {
		...
		if (sc.id == 'trx_columns' || sc.id == 'trx_skills' || sc.id == 'trx_team' || sc.id == 'trx_price_table') {	// Determine "count" parameter
			...
			tab.siblings('div').each(function() {
				...
				var merge = parseInt(item_tab.find('[data-param="colspan"]').val());
...
function themerex_shortcodes_show_tabs(field) {
	...
	if (isset(field.children)) {
		...
		output += THEMEREX_shortcodes_tab_clone_tab.replace(/{id}/g, 'add').replace('{icon}', 'plus').replace('{title}', '');
	}
	output += '</ul>';
	return output;
}
		...
		for (var param_num in field['params'][clone_num]) {
			...
			case 'media':
				...
				if (!empty(param['value'])) {
					...
					output += '<a class="themerex_options_image_preview" rel="prettyPhoto" target="_blank" href="' + param['value'] + '">' + (inList('jpg,png,gif', fext, ',') ? '<img src="'+param['value']+'" alt="" />' : '<span>'+fname+'</span>') + '</a>';
				}
			break;
			...
			case 'range':
				...
				if (param['value'].indexOf(THEMEREX_shortcodes_delimiter) == -1)
					param['value'] = Math.min(param['max'], Math.max(param['min'], param['value']));
				sliders = param['value'].split(THEMEREX_shortcodes_delimiter);
			...
			case "color":
				output += '<input class="themerex_options_input themerex_options_input_color"'
					+ ' name="' + id + '"'
					+ ' id="' + id + '"'
					+ ' type="text"'
					+ ' value="' + param['value'] + '"'
