...
class themerex_top10_widget extends WP_Widget {
	...
	function themerex_top10_widget() {
		...
		/* Create the widget. */
		parent::__construct('themerex-top10-widget', __('ThemeREX - Top 10 Posts', 'themerex'), $widget_ops, $control_ops);
	}
	...
	function widget($args, $instance) {
		...
		$title_tabs = array(
			apply_filters('widget_title', isset($instance['title_author']) ? ($instance['title_author'] ? $instance['title_author'] : __('Author','themerex') ) : ''),
			apply_filters('widget_title', isset($instance['title_users']) ? ($instance['title_users'] ? $instance['title_users'] : __('Users','themerex') ) : '')
		);
				...
				while (have_posts()) {
					...
					$output .= '
									<div class="post_wrapper">
										<div class="post_title theme_title'.($show_counters==2 ? '' : ' title_padding').'"><a href="' . $post_link . '">' . $post_title . '</a></div>
					';
					if ($show_counters) {
		...
		if (!empty($output)) {
			...
			echo '
				<div class="top10_tabs'.($show_image ? '' : ' ordered_list').($show_image || $show_date || $show_author ? '' : ' flat_list').'">'
					. (!$reviews_second_hide ? '<div class="sc_tabs sc_tabs_effects"><ul class="tabs">' . ($reviews_first_author ? $tabs[0].$tabs[1] : $tabs[1].$tabs[0]) . '</ul>' : '')
					.$output
				.'</div></div>';
	...
	function form($instance) {
		...
		<p>
			...
			<input id="<?php echo  $this->get_field_id('title_author'); ?>" name="<?php echo  $this->get_field_name('title_author'); ?>" value="<?php echo  $title_author; ?>" style="width:100%;" />
		</p>

		<p>
			<label for="<?php echo  $this->get_field_id('title_users'); ?>"><?php _e('Users rating tab title:', 'themerex'); ?></label>
			<input id="<?php echo  $this->get_field_id('title_users'); ?>" name="<?php echo  $this->get_field_name('title_users'); ?>" value="<?php echo  $title_users; ?>" style="width:100%;" />
		</p>
